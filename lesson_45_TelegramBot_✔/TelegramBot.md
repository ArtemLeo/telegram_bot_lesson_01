...
- Встановимо необхідні залежності для роботи з ботом, зокрема бібліотеку `aiogram`, яка допоможе керувати ботом в
  `Telegram`.
- Отримаємо токен для бота за допомогою `@BotFather` у `Telegram`, що дозволить програмі взаємодіяти з `Telegram`.
- Створимо файл `config.py` для зберігатиметься токен бота. Це необхідно для захищеного зберігання налаштувань.
- Створимо головний файл бота (bot.py`), де будемо писати код, що керуватиме усіма функціями бота.
...
- Додамо меню з доступними командами, щоб користувачі могли легко зрозуміти, які функції доступні у боті.
- Після цього уроку у нас буде готовий базовий **Telegram-Bot** з початковими налаштуваннями та першою командою для
...
В наступному блоку ми налаштуємо базову структуру нашого бота і почнемо розбиратись з його основними командами.
...
- Вказуємо шлях до папки, де буде зберігатися проект.
- Вибираємо віртуальне оточення на базі `pip`.
- **PyCharm** автоматично створює віртуальне оточення при створенні нового проекту.
...
> 💡 Об’єкти, які є **константами** та які **заборонено змінювати**, в товаристві розробників Python, принято називати в
> стилі **UPPER_CASE**.
...
Імпортуємо наш токен з файлу `config.py`, а рядок з `getenv` та отримання токену закоментуємо.
...
В прикладі [QuickStart](https://docs.aiogram.dev/en/latest/#simple-usage), який ми скопіювали зі сторінки офіційної
...
- Рядок `@dp.message(CommandStart())` вказує диспетчеру, що огорнута функція (та, що нижче оголошується) буде
  викликатися при отриманні повідомлення із текстом `/start`.
- **CommandStart** - це заготовлений клас, екземпляр якого є фільтром для специфічного повідомлення.
- В даному випадку використовується метод диспетчера `message`, який реалізує шаблон проєктування Decorator над функцією
  `command_start_handler`, а саме `TelegramEventObserver`, тобто він викликає функцію під час настання події.
- Також цей метод приймає в якості аргументів різноманітні фільтри для обробки специфічних даних, CommandStart якраз є
  таким прикладом.
...
- **Docstring** містить інформацію про роботу функції, а саме: **Цей обробник отримує повідомлення із командою `/start`
  **.
...
- Процес потребує очікування відповіді від серверу телеграму, про результат виконання запиту.
- Ключове слова `await` надає можливість процесору нашого комп'ютера не очікувати відповіді, виконувати інші задачі та
  повернувся до цього рядку коли буде готовий результат.

```python
await message.answer(f"Hello, {html.bold(message.from_user.full_name)}!")
```

Наступний код містить реалізацію обробника всіх повідомлень без фільтру.
...
Змінимо обробник `command_start_handler` для наших потреб.

Створимо власний обробник з фільтром для команди `/start` - змінимо наступний рядок в секції імпортів.

```python
# Рядок
from aiogram.filters import CommandStart

# необхідно змінити на наступний рядок
from aiogram.filters import CommandStart, Command
```

Це дозволить використовувати кастомні фільтри в обробниках.
...
- Переходимо в телеграм канал нашого бота.
- Натискаємо кнопку `start` або вручну пишемо боту повідомлення з текстом `/start`
...
- Створення проекту та підготовка середовища:
    - Створили окрему папку для проекту та ініціалізували віртуальне середовище.
    - Це дозволяє ефективно організувати залежності проекту та зробити код більш незалежним від системних налаштувань.
- Встановлення та налаштування бібліотеки aiogram:
    - Ми встановили бібліотеку aiogram, яка є основним інструментом для розробки асинхронних телеграм-ботів.
    - Aiogram дозволить нам легко обробляти команди та повідомлення користувачів.
- Отримання токена та налаштування бота.
    - Ми зареєстрували бота у Telegram за допомогою @BotFather та отримали унікальний токен.
    - Це ключ, який дозволяє нашому коду взаємодіяти з платформою Telegram.
- Створення конфігурації та головного файлу бота.
    - Ми створили файл config.py для збереження токена, що робить проект більш захищеним та зручним у налаштуванні.
    - У головному файлі bot.py ми написали перший код для нашого бота.
- Реалізація команди /start.
    - Ми створили першу команду /start, яка вітає користувача і коротко пояснює функціональність бота.
    - Це допоможе користувачам зрозуміти, що бот може робити та як ним користуватися.

Після цього уроку у нас вже є основи телеграм-бота, який відповідає на перші команди.

Далі ми будемо додавати нові функції та поступово наближатись до завершення проекту.
